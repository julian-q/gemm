...
int main(int argc, char **argv) {
  ...
  std::unordered_map<int, float> cuBLAS_GFLOPS = { // a100
    {128, 13.1},
    {256, 69.2},
    {512, 6971.9},
    {1024, 12453.4},
  ...
  int repeat_times = 10; // set to 50 for best results
  for (int size : SIZE) {
    m = n = k = size;
    ...
    long flops = 2 * m * n * k;
    printf(
        "Average elapsed time: (%7.6f) s, performance: (%7.1f) GFLOPS (%%cuBLAS: %7.2f%%). size: "
        "(%ld).\n",
        elapsed_time / repeat_times,
        (repeat_times * flops * 1e-9) / elapsed_time,
        ((repeat_times * flops * 1e-9) / elapsed_time) / cuBLAS_GFLOPS[m] * 100,
        m);
    fflush(stdout);
